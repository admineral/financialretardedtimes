...
const ArticleSchema = z.object({
  articles: z.array(z.object({
    ...
    timestamp: z.string().optional().describe('Approximate time period of the discussion'),
...
const SummarySchema = z.object({
  ...
  timespan: z.object({
    from: z.string(),
    to: z.string()
  }).describe('Time range of the analyzed messages')
})
...
export async function POST(request: NextRequest) {
  ...
  console.log('[SUMMARIZE API] OpenAI API Key:', apiKey ? `Found (${apiKey.slice(0, 8)}...${apiKey.slice(-4)})` : 'NOT FOUND')
  ...
  try {
    ...
    if (mode === 'articles') {
      // Generate structured newspaper articles
...
export async function GET(request: NextRequest) {
  ...
  const messageLimit = parseInt(searchParams.get('limit') || '200', 10)
  
  // Redirect to POST handler logic
  const fakeRequest = {
    json: async () => ({ mode, messageLimit })
  } as NextRequest
